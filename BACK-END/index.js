...
resp.send(result);
  ...
  app.get("/register/:id",async(req,resp)=>{
  ...
  app.get("/investor/:id",async(req,resp)=>{
    let result = await Sponser.findOne({_id:req.params.id});
    if(result){
      resp.send(result)
    }else{
      resp.send({result:"No Result Found"})
  ...
  app.post('/payments', async (req, resp) => {
